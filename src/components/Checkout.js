  ...
  return (
    <Box display="flex" flexDirection="column">
      ...
      <Stack direction="row" my="1rem">
        <Button
          variant="contained"
          onClick={e=>{addAddress(token,newAddress)}}
        >
          Add
        </Button>
        <Button
...
//  const [selected,setSelected]=useState("")
  ...
  const displayaddress=(e)=>{
       
      return (
        <Box id={e._id}  className= {`address-item ${addresses.selected===e._id? "selected":"not-selected"}`}  onClick={(p)=>{setAddresses({...addresses,selected:p.target.id}) } }  key={e._id}   >
          {e.address} <button onClick={( )=>{deleteAddress(token,e._id ) }} className="DeleteBtn"><Delete fontSize="small" color="primary" /><span style={{fontSize:"1rem",curser:"pointer"}}>DELETE</span> </button>
        </Box>
      )
  ...
  const deleteAddress = async (token, addressId) => {
    try {
      ...
      let response= await axios.delete(`${config.endpoint}/user/addresses/${addressId}` ,{ headers:{
